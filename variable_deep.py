...
class VariableDepth(Player):
		...
		def eval_board(board) -> float:
			...
			for square, piece in board.piece_map().items():
				...
				if piece.piece_type == chess.PAWN:
					...
					val = 1000
				if len(attackers) != 0 and piece.color == self.color:
					val = val / 10
			...
			while len(moveSearch) is not 0:
				...
				if im.moveNum == (deep - 1):
					if trueWin < 15 and deep < 21:
						...
						mS.append(msN)
						deep = deep + 1
					elif initialMove in move_actions:
						winningMove.add(im)
						continue
				...
				for move in legalMoves:
					...
					if len(mS[mv.moveNum]) > (1000 if ((mv.moveNum%2) != 0) else 25):
						mS[mv.moveNum].pop(0)

				while len(winningMove) > 100:
					winningMove.pop(0)
	...
	def handle_move_result(self, requested_move: Optional[chess.Move], taken_move: Optional[chess.Move],
		...
		print ("var_deep: End move ", self.moveNum, " -- approx ", self.approxTime, " seconds left")
